...
// Update how many LSB bits are masked out from raw sensor data (0=off, 2/3/4=drop noise).
void setRawMaskBits(uint8_t bits)
{
  if (bits == 2 || bits == 3 || bits == 4)
    ...
    g_rawMaskBits = 0;
  g_liveLastMs = 0; // force refresh path
}
...
{
  // Quick stationary noise scan to derive baseline magnitude (gravity removed).
  ...
  if (!lis.readRawAligned(axRaw, ayRaw, azRaw))
  {
    ...
    // Drop lowest bits to reduce quantization noise
    if (g_rawMaskBits)
    {
    ...
    float diff = mag - GRAVITY_MPS2; // remove 1g gravity to isolate vibration
...
{
  // Enable/disable realtime ISO20816 mode and optionally set LSB mask, then run noise scan.
  if (g_recording || g_calibratingStatic || g_calibrating6)
  {
    server.send(409, "text/plain", "Busy");
    return;
...
void handleApiLive()
{
  // Live endpoint: standard preview when realtime disabled; ISO20816 aggregates when enabled.
  if (g_recording || g_calibratingStatic || g_calibrating6)
  {
  ...
  float avgVel1s = (float)(sumVelMag * invRt) * 1000.0f;  // mm/s
  float avgDisp1s = (float)(sumDispMag * invRt) * 1000.0f; // mm
